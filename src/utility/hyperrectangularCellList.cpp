 ...
 */
hyperrectangularCellList::hyperrectangularCellList(double a, BoxPtr _box)
    {
    useGPU = false;
    Nmax = 2;
    Box = _box;
 ...
 */
void hyperrectangularCellList::setGridSize(double a)
    {
    dVec bDims;
...
void hyperrectangularCellList::computeAdjacentCells(int width)
    ...
    while(iVecIterate(it,min,max))
        ...
        for (int dd = 0; dd < DIMENSION; ++dd)
            {
            double temp = gridCellSizes.x[dd]*(std::max(abs(gridPosition.x[dd])-1.,0.0));
            rmin += temp*temp;
            }
        rmin = sqrt(rmin);
