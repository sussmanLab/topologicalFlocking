...
/*!
 * gives access to
Box.setBoxDimensions(vector<double> dimensions);
...
struct periodicBoundaryConditions
    ...
    public:
        ...
        HOSTDEVICE periodicBoundaryConditions(double sideLength){setHyperCubic(sideLength);};
        ...
        HOSTDEVICE void setHyperCubic(double sideLength);
...
void periodicBoundaryConditions::setHyperCubic(double sideLength)
    {
    for (int dd = 0; dd < DIMENSION; ++dd)
        {
